...
describe("oauth-state-service", () => {
  it("persists the state with its code verifier", async () => {
    ...
    const { state } = await createOAuthState(db, { applicationId: "app-b", codeVerifier: "verifier" });

    expect(state).toBeTruthy();
    const data = create.mock.calls[0]![0].data;
    expect(data.state).toBe(state);
    expect(data.codeVerifier).toBe("verifier");
    expect(data.applicationId).toBe("app-b");
  ...
  it("returns the verifier for a valid state and consumes it", async () => {
    ...
    const result = await consumeOAuthState(db, "state-1", "app-b");
  ...
  it("returns null for an expired state", async () => {
    ...
    expect(await consumeOAuthState(db, "state-1", "app-b")).toBeNull();
  });
...
describe("activity-service", () => {
  it("writes an activity row with correlation id", async () => {
    ...
    await writeActivity(db, { applicationId: "app-b", event: "code_received", correlationId: "corr-1" });
