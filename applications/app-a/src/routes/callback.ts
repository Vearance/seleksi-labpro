...
export async function callbackRoutes(server: FastifyInstance): Promise<void> {
  server.get("/callback", async (request, reply) => {
    ...
    try {
      tokenRes = await fetch(`${server.config.AUTH_SERVER_INTERNAL_URL}/oauth/token`, {
        ...
        body: JSON.stringify({
          ...
          code,
          client_id: server.config.APP_A_CLIENT_ID,
          client_secret: server.config.APP_A_CLIENT_SECRET,
          redirect_uri: `${server.config.APP_A_PUBLIC_URL}/callback`,
    ...
    reply.setCookie(COOKIE_NAMES.appA, token, {
      httpOnly: true,
      sameSite: "lax",
      path: "/",
      maxAge: ttlSeconds,
